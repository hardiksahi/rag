...
def _set_uniqueness_constraints(tx, node):
    ## Ensure unique property(id) for variable n of type node
    query = f"CREATE CONSTRAINT unique_{node} IF NOT EXISTS FOR (n:{node}) REQUIRE n.id IS UNIQUE"
    _ = tx.run(query, {})
...
def load_hospital_graph_from_csv() -> None:
    ...
    LOGGER.info("Setting uniquensess constraints on nodes")
    ...
    with driver.session(database="neo4j") as session:
        query = f"""LOAD CSV
        WITH HEADERS FROM '{HOSPITALS_CSV_PATH}' AS hospitals
        MERGE (h:Hospital {{id: toInteger(hospitals.hospital_id), name: hospitals.hospital_name, state_name: hospitals.hospital_state}});
        """
        _ = session.run(query, {})
    ...
    with driver.session(database="neo4j") as session:
        ...
        MERGE (p:Physician {{id: toInteger(physicians.physician_id), name: physicians.physician_name, dob: physicians.physician_dob, grad_year: physicians.physician_grad_year, school: physicians.medical_school, salary: toFloat(physicians.salary)}});
        """
        _ = session.run(query, {})

    LOGGER.info(f"Loading visit nodes")
    with driver.session(database="neo4j") as session:
        query = f"""
        LOAD CSV WITH HEADERS FROM '{VISITS_CSV_PATH}' AS visits
        MERGE (v:Visit {{id: toInteger(visits.visit_id),
        room_number: visits.room_number, admission_type: visits.admission_type, admission_date: visits.date_of_admission,
        test_results: visits.test_results, status: visits.visit_status}})
    ...
    with driver.session(database="neo4j") as session:
        ...
        MERGE (p:Patient {{id: toInteger(patients.patient_id), name: patients.patient_name, sex: patients.patient_sex, dob: patients.patient_dob, blood_type: patients.patient_blood_type}});
        """
    ...
    with driver.session(database="neo4j") as session:
        ...
        ON CREATE SET r.service_date=row.discharge_date, r.billing_amount=toFloat(row.billing_amount)
        ON MATCH SET r.service_date=row.discharge_date, r.billing_amount=toFloat(row.billing_amount)
        """
        _ = session.run(query, {})
    ...
    with driver.session(database="neo4j") as session:
        query = f"""
        LOAD CSV WITH HEADERS
        FROM '{VISITS_CSV_PATH}' as rows
        MATCH (source: Hospital {{id:toInteger(trim(rows.hospital_id))}})
        MATCH (target: Physician {{id:toInteger(trim(rows.physician_id))}})
